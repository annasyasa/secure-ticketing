...
class AuthorizationTest extends TestCase
{
    ...
    protected User $user; // <--- Tambahkan ini

    protected function setUp(): void // <--- Tambahkan method ini
    {
        parent::setUp();
    ...
    public function test_user_cannot_edit_other_users_ticket(): void
{
    ...
    // Gunakan actingAs dulu
    $response = $this->actingAs($userB)->put("/tickets/{$ticket->id}", [
        'title' => 'Hacked',
        'description' => 'Should fail',
        'priority' => 'high',
    ]);

    // Jika masih dapat 419, pastikan route /tickets/{id} 
    // memang menerima method PUT (bukan PATCH)
    $response->assertStatus(403);
}
...
{
    $user = User::factory()->create(); // Buat user lokal

    $response = $this->actingAs($user)->post('/tickets', [ // Gunakan $user, bukan $this->user
        'title' => 'Ticket Valid untuk Testing',
        'description' => 'Deskripsi yang cukup panjang minimal dua puluh karakter.',
        'priority' => 'medium',
    ]);
